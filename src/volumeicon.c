...
typedef struct {
	...
	GtkRadioButton *logarithmic_scale_radiobutton;
...
static void preferences_logarithmic_scale_radiobutton_toggled(
    GtkToggleButton *togglebutton, gpointer user_data)
{
	gboolean use_logarithmic_scale =
	    gtk_toggle_button_get_active(togglebutton);
	config_set_use_logarithmic_scale(use_logarithmic_scale);
...
static void
preferences_use_horizontal_slider_checkbutton_toggled(GtkCheckButton *widget,
                                                      gpointer user_data)
{
...
static void
preferences_show_sound_level_checkbutton_toggled(GtkCheckButton *widget,
                                                 gpointer user_data)
{
	gboolean active = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget));
                                               ...
                                               gpointer user_data)
{
	...
	if(gtk_combo_box_get_active_iter(gui->theme_combobox, &iter)) {
		gchar *theme;
		gtk_tree_model_get(GTK_TREE_MODEL(gui->theme_store), &iter, 0, &theme,
		                   -1);
                                  ...
                                  gchar *path, gpointer user_data)
{
	...
	if(gtk_tree_model_get_iter_from_string(GTK_TREE_MODEL(gui->hotkey_store),
	                                       &iter, path)) {
		gboolean enabled = FALSE;
		gchar *accel_name = NULL;
		enum HOTKEY hotkey = 0;
		gtk_tree_model_get(GTK_TREE_MODEL(gui->hotkey_store), &iter, 1,
		                   &accel_name, 2, &hotkey, 3, &enabled, -1);
		enabled = !enabled;
		...
		if(enabled &&
		   !keybinder_bind(accel_name, hotkey_handle, (void *)hotkey)) {
			g_fprintf(stderr, "Failed to bind %s\n", accel_name);
		}
		else {
			gtk_list_store_set(GTK_LIST_STORE(gui->hotkey_store), &iter, 3,
			                   enabled, -1);
			switch(hotkey) {
			case UP:
                                            ...
                                            gpointer user_data)
{
	...
	if(gtk_tree_model_get_iter_from_string(GTK_TREE_MODEL(gui->hotkey_store),
		...
		gtk_tree_model_get(GTK_TREE_MODEL(gui->hotkey_store), &iter, 1,
		                   &old_value, 2, &hotkey, 3, &enabled, -1);

		if(enabled &&
		   !keybinder_bind(new_value, hotkey_handle, (void *)hotkey)) {
			g_fprintf(stderr, "Failed to bind %s\n", new_value);
		}
		else {
			gtk_list_store_set(GTK_LIST_STORE(gui->hotkey_store), &iter, 1,
			                   new_value, -1);
...
static void preferences_use_transparent_background_checkbutton_toggled(
    GtkCheckButton *widget, gpointer user_data)
{
	gboolean active = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget));
...
static void
preferences_show_notification_checkbutton_toggled(GtkCheckButton *widget,
                                                  gpointer user_data)
{
	gboolean active = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget));
...
static void preferences_notification_combobox_changed(GtkComboBox *widget,
                                                      gpointer user_data)
{
	GtkTreeIter iter;
...
#define getobj(x) gtk_builder_get_object(gui->builder, x)
	...
	gui->linear_scale_radiobutton =
	    GTK_RADIO_BUTTON(getobj("linear_scale_radiobutton"));
	gui->logarithmic_scale_radiobutton =
	    GTK_RADIO_BUTTON(getobj("logarithmic_scale_radiobutton"));
	...
	gui->mmb_mute_radiobutton =
	    GTK_RADIO_BUTTON(getobj("mmb_mute_radiobutton"));
	gui->mmb_mixer_radiobutton =
	    GTK_RADIO_BUTTON(getobj("mmb_mixer_radiobutton"));
	gui->use_horizontal_slider_checkbutton =
	    GTK_CHECK_BUTTON(getobj("use_horizontal_slider"));
	gui->show_sound_level_checkbutton =
	    GTK_CHECK_BUTTON(getobj("show_sound_level"));
	gui->hotkey_accel = GTK_CELL_RENDERER(getobj("hotkey_cellrendereraccel"));
	gui->hotkey_toggle =
	    GTK_CELL_RENDERER_TOGGLE(getobj("hotkey_cellrenderertoggle"));
	gui->use_transparent_background_checkbutton =
	    GTK_CHECK_BUTTON(getobj("use_transparent_background"));
	gui->show_notification_checkbutton =
	    GTK_CHECK_BUTTON(getobj("show_notifications"));
	gui->notification_combobox =
	    GTK_COMBO_BOX(getobj("notification_combobox"));
	gui->notification_store =
	    GTK_LIST_STORE(getobj("notification_type_model"));
#undef getobj
	...
	if(config_get_use_logarithmic_scale()) {
		gtk_toggle_button_set_active(
		    GTK_TOGGLE_BUTTON(gui->logarithmic_scale_radiobutton), TRUE);
	}
	else {
		gtk_toggle_button_set_active(
		    GTK_TOGGLE_BUTTON(gui->linear_scale_radiobutton), TRUE);
	}

	if(config_get_left_mouse_slider()) {
		gtk_toggle_button_set_active(
		    GTK_TOGGLE_BUTTON(gui->slider_radiobutton), TRUE);
	}
	else {
	...
	if(config_get_middle_mouse_mute()) {
		gtk_toggle_button_set_active(
		    GTK_TOGGLE_BUTTON(gui->mmb_mute_radiobutton), TRUE);
	}
	else {
		gtk_toggle_button_set_active(
		    GTK_TOGGLE_BUTTON(gui->mmb_mixer_radiobutton), TRUE);
	}

	gtk_toggle_button_set_active(
	    GTK_TOGGLE_BUTTON(gui->use_horizontal_slider_checkbutton),
	    config_get_use_horizontal_slider());

	gtk_toggle_button_set_active(
	    GTK_TOGGLE_BUTTON(gui->show_sound_level_checkbutton),
	    config_get_show_sound_level());

	gtk_toggle_button_set_active(
	    GTK_TOGGLE_BUTTON(gui->use_transparent_background_checkbutton),
	...
	gtk_list_store_set(gui->hotkey_store, &tree_iter, 0, _("Volume Up"), 1,
	                   config_get_hotkey_up(), 2, (int)UP, 3,
	                   config_get_hotkey_up_enabled(), -1);
	gtk_list_store_append(gui->hotkey_store, &tree_iter);
	gtk_list_store_set(gui->hotkey_store, &tree_iter, 0, _("Volume Down"), 1,
	                   config_get_hotkey_down(), 2, (int)DOWN, 3,
	                   config_get_hotkey_down_enabled(), -1);
	gtk_list_store_append(gui->hotkey_store, &tree_iter);
	gtk_list_store_set(gui->hotkey_store, &tree_iter, 0, _("Mute"), 1,
	                   config_get_hotkey_mute(), 2, (int)MUTE, 3,
	                   config_get_hotkey_mute_enabled(), -1);
...
#ifdef COMPILEWITH_NOTIFY
	gtk_list_store_append(gui->notification_store, &tree_iter);
	gtk_list_store_set(gui->notification_store, &tree_iter, 0, _("libnotify"),
	                   1, (gint)NOTIFICATION_LIBNOTIFY, -1);
#endif
	...
	g_signal_connect(G_OBJECT(gui->window), "destroy",
	                 G_CALLBACK(preferences_window_destroy), NULL);
	g_signal_connect(G_OBJECT(gui->window), "delete-event",
	                 G_CALLBACK(preferences_window_delete_event), NULL);
	g_signal_connect(G_OBJECT(gui->close_button), "clicked",
	                 G_CALLBACK(preferences_close_button_clicked), NULL);
	g_signal_connect(G_OBJECT(gui->device_combobox), "changed",
	                 G_CALLBACK(preferences_device_combobox_changed), NULL);
	g_signal_connect(G_OBJECT(gui->channel_combobox), "changed",
	                 G_CALLBACK(preferences_channel_combobox_changed), NULL);
	g_signal_connect(G_OBJECT(gui->theme_combobox), "changed",
	                 G_CALLBACK(preferences_theme_combobox_changed), NULL);
	g_signal_connect(G_OBJECT(gui->volume_adjustment), "value-changed",
	                 G_CALLBACK(preferences_volume_adjustment_changed), NULL);
	g_signal_connect(G_OBJECT(gui->mixer_entry), "changed",
	                 G_CALLBACK(preferences_mixer_entry_changed), NULL);
	g_signal_connect(
	...
	g_signal_connect(
	    ...
	    G_CALLBACK(preferences_logarithmic_scale_radiobutton_toggled), NULL);
	g_signal_connect(G_OBJECT(gui->mute_radiobutton), "toggled",
	                 G_CALLBACK(preferences_mute_radiobutton_toggled), NULL);
	g_signal_connect(G_OBJECT(gui->mmb_mixer_radiobutton), "toggled",
	                 G_CALLBACK(preferences_mmb_radiobutton_toggled), NULL);
	g_signal_connect(
	    G_OBJECT(gui->use_horizontal_slider_checkbutton), "toggled",
	    G_CALLBACK(preferences_use_horizontal_slider_checkbutton_toggled),
	    NULL);
	g_signal_connect(
	    G_OBJECT(gui->show_sound_level_checkbutton), "toggled",
	    G_CALLBACK(preferences_show_sound_level_checkbutton_toggled), NULL);
	g_signal_connect(G_OBJECT(gui->hotkey_accel), "accel-edited",
	                 G_CALLBACK(preferences_hotkey_accel_edited), NULL);
	g_signal_connect(G_OBJECT(gui->hotkey_toggle), "toggled",
	                 G_CALLBACK(preferences_hotkey_toggle_toggled), NULL);
	g_signal_connect(
	    G_OBJECT(gui->use_transparent_background_checkbutton), "toggled",
	    G_CALLBACK(preferences_use_transparent_background_checkbutton_toggled),
	    NULL);
	g_signal_connect(
	    G_OBJECT(gui->show_notification_checkbutton), "toggled",
	    G_CALLBACK(preferences_show_notification_checkbutton_toggled), NULL);
	g_signal_connect(G_OBJECT(gui->notification_combobox), "changed",
	                 G_CALLBACK(preferences_notification_combobox_changed),
	                 NULL);
...
static gboolean scale_timeout(gpointer data)
{
	...
	root_window =
	    gdk_screen_get_root_window(gtk_widget_get_screen(m_scale_window));
	device_manager =
	    gdk_display_get_device_manager(gdk_window_get_display(root_window));
	pointer = gdk_device_manager_get_client_pointer(device_manager);
	gdk_window_get_device_position(root_window, pointer, &x, &y, NULL);

	if(scale_point_in_rect(&window, x, y) ||
	   scale_point_in_rect(&icon, x, y)) {
...
static void volume_icon_load_icons()
{
	...
	for(i = 0; i < ICON_COUNT; i++) {
		gchar *icon_path =
		    g_strdup_printf(ICONS_DIR "/%s/%d.png", theme, i + 1);
...
static gboolean on_draw(GtkWidget *widget, cairo_t *cairo_context,
                        gpointer user_data)
{
	render_widget(cairo_context, gtk_widget_get_allocated_width(widget),
...
static void scale_setup()
{
	...
	if(config_get_use_horizontal_slider()) {
		m_scale = gtk_scale_new_with_range(GTK_ORIENTATION_HORIZONTAL, 0.0,
		                                   100.0, 1.0);
	}
	else {
		m_scale = gtk_scale_new_with_range(GTK_ORIENTATION_VERTICAL, 0.0,
		                                   100.0, 1.0);
		gtk_range_set_inverted(GTK_RANGE(m_scale), TRUE);
	}
	gtk_scale_set_draw_value(GTK_SCALE(m_scale),
	                         config_get_show_sound_level());
	   ...
	   config_get_use_transparent_background()) {
		...
		if(visual) {
			...
			gdk_window_set_background_pattern(
			    gtk_widget_get_window(GTK_WIDGET(m_scale_window)), NULL);
			gtk_window_set_type_hint(GTK_WINDOW(m_scale_window),
			                         GDK_WINDOW_TYPE_HINT_DOCK);

			g_signal_connect(G_OBJECT(m_scale_window), "draw",
			                 G_CALLBACK(on_draw), NULL);
			g_signal_connect(G_OBJECT(m_scale_window), "configure-event",
			                 G_CALLBACK(on_configure), NULL);

			update_widget(
			    GTK_WIDGET(m_scale_window),
...
// Setup OSD Notification
	...
	gtk_box_pack_start(GTK_BOX(hbox), GTK_WIDGET(m_popup_icon), FALSE, FALSE,
	                   0);
	gtk_box_pack_start(GTK_BOX(hbox), GTK_WIDGET(m_pbar), TRUE, TRUE, 0);
